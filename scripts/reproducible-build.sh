#!/usr/bin/env bash
# Enforces invariant 11 of CLAUDE.md: a released binary can be rebuilt, by
# somebody who does not trust us, from the tag it claims to come from.
#
# WHY THIS IS WORTH A GATE RATHER THAN A SENTENCE
#
# agent-conform decides whether somebody else's payload conforms to this
# contract. Its output is a verdict about their system, and a verdict is worth
# what its checker is worth, so the checker itself is the first thing a careful
# reader will want to pin down. "The source is open" does not answer them: it
# always was. The answer is that they can rebuild the release and get the same
# bytes.
#

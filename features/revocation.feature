Feature: A revocation list something actually consults
  ...
  # @test:TestARevocationNamingAnyPartyInTheChainRefusesTheToken
  Scenario: A revocation names a party wherever that party stands in the chain
    ...
    # field. Until then the verifier asked the list about `sub` alone, so an
    # entry naming an agent in `act` matched nothing and revoked nobody; every
    # earlier test had planted the agent as the argument directly.

  # @test:TestRevocationIsNotConsultedForATokenWhoseChainIsMalformed

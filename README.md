...
![tests](https://img.shields.io/badge/tests-141-brightgreen.svg)
...
- **Talks to**: imported by **Idryx**, **Wardryx**, **Mockryx**, **Qryx**, **heraldyx**, **scopyx**, **terraform-provider-taipan** and **vouchryx**, so all eight speak the same identity and event language as **TokenFuse** and **Engram**, which reach it through their own languages.
...
| `delegation` | RFC 8693 + RFC 9449 | the delegation TOKEN: signing, offline verification against a held key set, DPoP sender-constraint, and the `act` chain. `vouchryx` issues with it; every enforcement point checks with it. Standard library only |
...
- [x] `v0.6.0` tagged, which is what the install lines above pin. The library
  ...
  schema, and the event schemas gained SPEC 5.1's depth bound, so a stream with
  a chain over 32 entries now fails where it used to pass. That is a minor
  rather than a patch for exactly that reason. It also drops a
  `darwin/arm64` build of `agent-conform` that had been committed at the module
